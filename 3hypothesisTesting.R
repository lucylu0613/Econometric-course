...
tc <- qt(1-alpha/2, df)	# critical value of t distribution qt(probability minius right tail,degree od freedom)
lowb <- b2-tc*seb2  # lower bound of confidence interval
upb <- b2+tc*seb2   # upper bound
lowb; upb;
ci <- confint(mod1) # default is 95% confidence interval can change parameter called level
#?confint
print(ci)
ci2 <- confint(mod1,level=0.99)
print(ci2)
...
sig2 = 100;
N = 30;
n = 100; # interate how many time
x = runif(N, 0, 10); #just generate some x
...
for(idx in 1: n)
{
  ...
  upb1[idx] <- b1+tc*seb1
  lowb2[idx] <- b2-tc*seb2
  upb2[idx] <- b2+tc*seb2

ifCover = (beta1>lowb1[idx]) * (upb1[idx]>beta1)
# check whether beta1 is in the confidence interval
if(ifCover){
